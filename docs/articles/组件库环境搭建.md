...
本文将使用Vue3和TypeScript来编写一个组件库，使用Vite+Vue3来对这个组件库中的组件进行调试，最后使用vite来对组件库进行打包并且发布到npm上。最终的产物是一个名为@colin-vue3/peace-ui的组件库。
...
既然它是一个包，所以我们新建utils目录后就需要初始化它，让它变成一个包；终端进入utils文件夹执行：pnpm init 然后会生成一个package.json文件；这里需要改一下包名，我这里将name改成@colin-vue3/utils表示这个utils包是属于@colin-vue3这个组织下的。所以记住发布之前要登录npm新建一个组织；例如@colin-vue3

```
{
  "name": "@colin-vue3/utils",
  "version": "1.0.0",
  "description": "",
  "main": "index.ts",
  "scripts": {
...
* 组件库包(这里命名为@colin-vue3/peace-ui)
...
```
import {testfun} from '@colin-vue3/peace-ui/utils'
...
```
{
  ...
  "license": "ISC",
  "dependencies": {
    "@colin-vue3/utils": "workspace:^1.0.1"
  }
}

```

你会发现它的依赖@colin-vue3/utils对应的版本为：workspace:^1.0.0；因为pnpm是由workspace管理的，所以有一个前缀workspace可以指向utils下的工作空间从而方便本地调试各个包直接的关联引用。
...
所以这里我们直接在examples执行：pnpm i @colin-vue3/peace-ui

此时你就会发现packages.json的依赖多了个 

```
"@colin-vue3/peace-ui": "workspace:^1.0.0"
```
...
import { Button } from '@colin-vue3/peace-ui'
</script>
```
...
![peace.jpg](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/cd1f139224304b159fdf12ad5836a2c0~tplv-k3u1fbpfcp-watermark.image?)
...
做了那么多终于到发布的阶段了；其实npm发包是很容易的，就拿我们的组件库@colin-vue3/peace-ui举例吧

发布之前记得到[npm](https://www.npmjs.com/)官网注册个账户,如果你要发布@xx/xx这种包的话需要在npm新建个组织组织组织名就是@后面的，比如我建的组织就是@colin-vue3/peace-ui,注册完之后你就可以发布了
...
```
import '@colin-vue3/peace-ui/es/style.css';
```
...
如果你不想一步步的搭建，想直接使用现成的话，你可以直接把项目clone下来->**[peace-ui第一版](https://gitee.com/lcyh/lc-peace-ui/tree/master)**,然后你只需要以下几步便可将其完成
